...
class CryptoDetector:
    ...
    def __init__(self):
        ...
        self.ambiguous_tickers = self._load_ambiguous_tickers()
        
        # Separate tickers into ambiguous and non-ambiguous
        self.non_ambiguous_tickers = [t for t in self.tickers if t not in self.ambiguous_tickers]
        self.ambiguous_ticker_list = list(self.ambiguous_tickers.keys())
        
        # Non-ambiguous ticker pattern: word boundary + ticker + word boundary (case insensitive)
        if self.non_ambiguous_tickers:
            ticker_pattern = r'\b(' + '|'.join(re.escape(t) for t in self.non_ambiguous_tickers) + r')\b'
            self.ticker_regex = re.compile(ticker_pattern, re.IGNORECASE)
        else:
            self.ticker_regex = None
        
        # Ambiguous ticker pattern: requires $ or # prefix
        if self.ambiguous_ticker_list:
            # Pattern: [$#] followed by ticker (case insensitive)
            ambiguous_pattern = r'[\$#](' + '|'.join(re.escape(t) for t in self.ambiguous_ticker_list) + r')\b'
            self.ambiguous_ticker_regex = re.compile(ambiguous_pattern, re.IGNORECASE)
        else:
            self.ambiguous_ticker_regex = None
        
        # Generic $ or # prefixed ticker pattern: catches ANY ticker with prefix (2-10 chars)
        # This catches new/unknown coins like $AVICI, $NKP, $PEAS, $HILO, $RVF
        self.generic_prefixed_ticker_regex = re.compile(r'[\$#]([A-Z]{2,10})\b')
        ...
        ambiguous_count = len(self.ambiguous_tickers) if self.ambiguous_tickers else 0
        self.logger.info(
            f"Crypto detector initialized ({ticker_count} tickers: "
            f"{len(self.non_ambiguous_tickers)} standard, {ambiguous_count} ambiguous, "
    ...
    def detect_mentions(self, text: str) -> list[str]:
        ...
        try:
            # Detect non-ambiguous ticker symbols (no prefix required)
            if self.ticker_regex:
                ...
                if tickers:
                    ...
                    self.logger.debug(f"Standard tickers detected: {[t.upper() for t in tickers]}")
            
            # Detect ambiguous ticker symbols (requires $ or # prefix)
            if self.ambiguous_ticker_regex:
                ambiguous_tickers = self.ambiguous_ticker_regex.findall(text)
                if ambiguous_tickers:
                    # Convert to uppercase and add to mentions
                    mentions.extend([t.upper() for t in ambiguous_tickers])
                    self.logger.debug(f"Ambiguous tickers detected (with prefix): {[t.upper() for t in ambiguous_tickers]}")
            
            # Detect generic $ or # prefixed tickers (catches unknown/new coins)
            generic_tickers = self.generic_prefixed_ticker_regex.findall(text)
            if generic_tickers:
                # Filter out already detected tickers to avoid duplicates
                new_tickers = [t.upper() for t in generic_tickers if t.upper() not in mentions]
                if new_tickers:
                    mentions.extend(new_tickers)
                    self.logger.debug(f"Generic prefixed tickers detected: {new_tickers}")
    ...
    def _load_tickers(self) -> list[str]:
        ...
        """
        try:
            # Config files are in the root config/ directory
            patterns_file = Path(__file__).parent.parent.parent / 'config' / 'crypto_patterns.json'
    ...
    def _load_crypto_keywords(self) -> list[str]:
        ...
        try:
            # Config files are in the root config/ directory
            keywords_file = Path(__file__).parent.parent.parent / 'config' / 'crypto_keywords.json'
    ...
    def _load_ambiguous_tickers(self) -> dict:
        """
        Load ambiguous tickers that require prefix detection.
        
        Returns:
            Dictionary mapping ticker symbol to metadata
        """
        try:
            # Config files are in the root config/ directory
            ambiguous_file = Path(__file__).parent.parent.parent / 'config' / 'ambiguous_tickers.json'
            
            if not ambiguous_file.exists():
                self.logger.warning(f"Ambiguous tickers file not found: {ambiguous_file}")
                return {}
            
            with open(ambiguous_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            ambiguous_tickers = data.get('ambiguous_tickers', {})
            
            # Filter to only those that require prefix
            requires_prefix = {
                ticker: info for ticker, info in ambiguous_tickers.items()
                if info.get('requires_prefix', False)
            }
            
            self.logger.info(f"Loaded {len(requires_prefix)} ambiguous tickers requiring prefix")
            return requires_prefix
            
        except Exception as e:
            self.logger.error(f"Error loading ambiguous tickers: {e}")
            return {}
