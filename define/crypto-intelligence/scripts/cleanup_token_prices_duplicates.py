"""
Script to remove duplicate rows from Token Prices sheet in Google Sheets.

This script identifies and removes duplicate token price entries based on
...
def cleanup_token_prices_duplicates():
    """Remove duplicate rows from Token Prices sheet."""
    logger = setup_logger('CleanupTokenPrices')
    ...
    try:
        ...
        # Get Token Prices sheet
        sheet = sheets_writer.sheets.get('Token Prices')
        if not sheet:
            logger.error("Token Prices sheet not found")
            return
        
        logger.info("Fetching all rows from Token Prices sheet...")
        ...
        logger.info("Clearing Token Prices sheet...")
        sheet.clear()
        ...
        logger.info(f"✓ Token Prices sheet now has {len(rows_to_keep) - 1} unique rows.")
...
if __name__ == "__main__":
    cleanup_token_prices_duplicates()
