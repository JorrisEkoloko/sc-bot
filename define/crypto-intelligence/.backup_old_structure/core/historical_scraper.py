...
from core.telegram_monitor import TelegramMonitor, MessageEvent
    ...
    async def _process_messages(self, messages: list, channel_name: str) -> int:
        ...
        for i, message in enumerate(messages, 1):
            try:
                ...
                self.logger.info(f"Cancellation detected at message {i}/{total}")
                raise
            except Exception as e:
                self.logger.error(f"Error processing message {message.id}: {e}")
                # Continue processing other messages
        
        self.logger.info(f"Completed processing {total} historical messages")
        return total
