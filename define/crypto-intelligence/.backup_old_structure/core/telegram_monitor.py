    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Ensure cleanup if connection was not successful
        ...
        try:
            ...
            async def handle_new_message(event):
                ...
                try:
                    ...
                    return  # Don't proceed to callback if extraction failed
                
                # Phase 2: Call the callback (separate error handling)
                if self.message_callback:
                    try:
                        ...
                        self.logger.error(
                            f"Callback error for message {message_id} from {channel_name}: {callback_error}",
                            exc_info=True
                        )
                        
                        # Track consecutive failures (after filtering out non-errors)
                        self._callback_failure_count += 1
                        
                        # Add exponential backoff for consecutive failures
                        if self._callback_failure_count >= 3:
                            backoff_time = min(60, 2 ** (self._callback_failure_count - 3))
                            self.logger.warning(f"Backing off for {backoff_time}s after {self._callback_failure_count} failures")
            ...
            if self._event_handler:
                ...
                try:
                    ...
                    self.logger.error(f"Failed to remove event handler: {e}")
                finally:
                    # Only clear if removal succeeded OR if we're certain it's safe
                    if removal_successful or not self.connected:
                        self._event_handler = None
                    self._monitoring_active = False
            else:
                self._monitoring_active = False
